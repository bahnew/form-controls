...
describe('ObsMapper', () => {
  ...
  it('should return value same as obs value', () => {
    obs.value = 'test value';

    expect(mapper.getValue(obs).value).toBe(obs.value);
  });

  it('should return comment same as obs comment', () => {
    obs.comment = 'test comment';

    expect(mapper.getValue(obs).comment).toBe(obs.comment);
  });

  it('should return empty children', () => {
    expect(mapper.getChildren(obs)).toHaveLength(0);
  });
  ...
  it('should return expected obsArray when getInitialObject be triggered', () => {
    ...
    expect(obsArray[0].concept).toBe(pulseConcept);
    expect(obsArray[0].formFieldPath).toBe('SingleObs.1/1-0');
  });
  ...
  it('should convert obs from record when getData be triggered', () => {
    ...
    expect(updatedObs.value).toBe(1);
    expect(updatedObs.comment).toBeUndefined();
    expect(updatedObs.interpretation).toBeUndefined();
    expect(updatedObs.formFieldPath).toBe(formFieldPath);
  });
  ...
  it('should get correct obs when given obs control with boolean type', () => {
    ...
    expect(updatedObs.value).toBe(false);
    expect(updatedObs.voided).toBe(false);
  });
  ...
  it('should trim spaces of value when given text control with string value', () => {
    ...
    expect(updatedObs.value).toBe(valueWithSpaces.trim());
  });
  ...
  it('should mark voided of obs as true when given obs control with complex media type and the value contains voided and also remove voided suffix from the value', () => {
    ...
    expect(updatedObs.voided).toBe(true);
    expect(updatedObs.value).toBe(expectedValue);
  });
  ...
  it('should not make the obs uuid for a voided record to undefined', () => {
    ...
    const updatedObs = mapper.getData(record);
    expect(updatedObs.uuid).toBe(complexDataSource.uuid);
  });
});
