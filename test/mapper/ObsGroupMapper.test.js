...
describe('ObsGroupMapper', () => {
  ...
  it('should return final object', () => {
    ...
    expect(observationGroup.groupMembers.length).toBe(2);
  ...
  it('should handle updateOldObsGroupFormFieldPathForBackwardCompatibility with empty observations', () => {
    ...
    const result = mapper.updateOldObsGroupFormFieldPathForBackwardCompatibility(control, [], 'parentPath');
  ...
  it('should handle updateOldObsGroupFormFieldPathForBackwardCompatibility with observations that include control prefix', () => {
    ...
    const result = mapper.updateOldObsGroupFormFieldPathForBackwardCompatibility(control, observations, 'parentPath');
  ...
  it('should return same amount obsGroups having add more property false and group members should not have obs group form field path', () => {
    ...
    expect(obsArray.length).toBe(observation.length);
    expect([obsArray[0].formFieldPath, obsArray[1].formFieldPath])
      .toEqual(expect.arrayContaining([firstObsGrpFormFieldPath, secondObsGrpFormFieldPath]));

    let obsGrp = obsArray.find(obj => obj.formFieldPath === firstObsGrpFormFieldPath);
    expect(obsGrp.groupMembers.length).toBe(1);
  ...
  it('should return same amount obsGroups having add more property false with parent form field path', () => {
    ...
    const obsArray = new ObsGroupMapper()
      .getInitialObject(formName, formVersion, control, observation, observation, 'Test1.1/1-0');
