...
describe('ObsListMapper', () => {
  ...
  it('should get obsList when getInitialObject with empty observation', () => {
    ...
    expect(obsLists[0].obs.concept).toBe(multipleSelectConcept);
    expect(obsLists[0].obs.formFieldPath).toBe(formFieldPath);
    expect(obsLists[0].obs.voided).toBe(false);
  });
  ...
  it('should save comment to obs when given record has comment', () => {
    ...
    expect(obsArray[0].comment).toBe(record.value.comment);
    expect(obsArray[1].comment).toBe(record.value.comment);
  });
  ...
  it('should get value with comment when given record has comment', () => {
    ...
    expect(value.comment).toBe(obs1WithComment.comment);
  });
  ...
  it('should clone previous observation from obsList of datasource', () => {
    ...
    expect(obs.observationDateTime).toBe('2018-12-17T07:32:35.000+0000');
  });
  ...
  it('should clone obs from dataSource when obsList is empty', () => {
    ...
    expect(obs.encounterDateTime).toBeUndefined();
  ...
  it('should handle obs without value in getValue', () => {
    const obsWithValue = new Obs({
      concept: multipleSelectConcept,
      formFieldPath,
      formNamespace: 'Bahmni',
      value: {
    ...
    const obsListDataMixed = new ObsList({
      obsList: List.of(obsWithValue, obsWithoutValue),
      formFieldPath,
      obs: new Obs({
        concept: multipleSelectConcept,
        formFieldPath,
  ...
  it('should return empty value when all obs have no value in getValue', () => {
    const obsWithoutValue1 = new Obs({
      concept: multipleSelectConcept,
      formFieldPath,
      formNamespace: 'Bahmni',
      value: null,
    ...
    const obsWithoutValue2 = new Obs({
      concept: multipleSelectConcept,
      formFieldPath,
      formNamespace: 'Bahmni',
      value: null,
    ...
    const obsListDataEmpty = new ObsList({
      obsList: List.of(obsWithoutValue1, obsWithoutValue2),
      formFieldPath,
      obs: new Obs({
        concept: multipleSelectConcept,
        formFieldPath,
  ...
  it('should handle null valueList in findObs', () => {
    const result = new ObsListMapper().findObs(null, 'some-uuid');

    expect(result).toBe(null);
  });

  it('should handle undefined valueList in findObs', () => {
    const result = new ObsListMapper().findObs(undefined, 'some-uuid');

    expect(result).toBeUndefined();
  });

  it('should handle fillEmptyData for addMore records', () => {
    const record = new ControlRecord({
      control: multipleSelectControl,
      formFieldPath: 'multipleSelect.1/2-1',
      value: { value: [] },
      dataSource: {
        formFieldPath: 'multipleSelect.1/2-0',
        obsList: List.of(obs1),
      },
    });
  ...
  it('should handle getData with voided record', () => {
    ...
    expect(obsArray[0].voided).toBe(true);
    expect(obsArray[0].value).toBeUndefined();
  });
});
