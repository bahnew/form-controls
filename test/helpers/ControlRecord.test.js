...
describe('Control Record', () => {
  ...
  describe('Single Layer Record', () => {
    ...
    it('should create single layer record given single layer metadata and empty data', () => {
      ...
      expect(controlRecordTree.children.get(0).formFieldPath).toBe(expectedFormFieldPath);
      expect(controlRecordTree.children.get(0).control.concept.uuid).toBe(obsConcept.uuid);
    });
    ...
    it('should update value when given value to obs control', () => {
      ...
      expect(updatedRecordTree.children.get(0).formFieldPath).toBe(formFieldPath);
  ...
  describe('Multiple layer Record', () => {
    ...
    it('should generate data from record when input obs is empty', () => {
      ...
      expect(obs.length).toBe(1);
      expect(obs[0].groupMembers.length).toBe(1);
    ...
    it('should remove value of all child records when a parent record is removed', () => {
      ...
      expect(updatedControlRecord.children.get(0).children.get(0).getValue()).toBeUndefined();
    });
    ...
    it('should generate data from record when input obs is nested section', () => {
      ...
      expect(controlRecordTree.children.size).toBe(1);
      expect(controlRecordTree.children.get(0).children.get(0).value.value).toBe(23);
  ...
  it('should generate record tree when section is add-more in given metadata with child obsControl',
      () => {
        ...
        expect(controlRecordTree.children.size).toBe(1);
        expect(controlRecordTree.children.get(0).children.get(0).value.value).toBe(120);
      });
  ...
  it('should generate record tree when section is add-more in given metadata with child obsGroupControl', () => {
    ...
    expect(sectionRecord.formFieldPath).toBe('section-add-more-with-obsGroup.1/1-0');
    const obsGroupRecord = sectionRecord.children.get(0);
    expect(obsGroupRecord.formFieldPath).toBe('section-add-more-with-obsGroup.1/1-0/2-0');
    expect(obsGroupRecord.children.get(0).formFieldPath).toBe('section-add-more-with-obsGroup.1/1-0/2-0/3-0');
