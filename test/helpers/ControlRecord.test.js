...
describe('Control Record', () => {
  ...
  describe('Single Layer Record', () => {
    ...
    it('should create single layer record given single layer metadata and empty data', () => {
      ...
      const controlRecordTree = new ControlRecordTreeBuilder().build(
        metadata,
        emptyObservation,
      );

      expect(controlRecordTree.children.size).toBe(1);
      expect(controlRecordTree.children.get(0).formFieldPath).toBe(
        expectedFormFieldPath,
      );
      expect(controlRecordTree.children.get(0).control.concept.uuid).toBe(
        obsConcept.uuid,
      );
    });
    ...
    it('should create single layer record given single layer metadata and data', () => {
      ...
      expect(controlRecordTree.children.get(0).value.value).toBe(1);
    });
    ...
    it('should update value when given value to obs control', () => {
      ...
      const updatedRecordTree = controlRecordTree.update(
        formFieldPath,
        updatedValue,
      );

      expect(updatedRecordTree.children.get(0).formFieldPath).toBe(
        formFieldPath,
      );
      expect(updatedRecordTree.children.get(0).value.value).toBe(1);
    });
    ...
    it('should generate data from record when input obs is empty', () => {
      ...
      expect(obs[0].value).toBe(newValue.value);
    });
  ...
  describe('Multiple layer Record', () => {
    ...
    it('should create multiple layer record given multiple layer metadata and empty data', () => {
      ...
      expect(node.get(0).formFieldPath).toBe(expectedFormFieldPath);
      expect(node.get(0).control.concept.uuid).toBe(obsGroupConcept.uuid);

      const subNode = node.get(0).children;
      expect(subNode.size).toBe(1);
      expect(subNode.get(0).formFieldPath).toBe(expectedSubFormFieldPath);
      expect(subNode.get(0).control.concept.uuid).toBe(obsConcept.uuid);
    });
    ...
    it('should create multiple layer record given multiple layer metadata and data', () => {
      ...
      expect(subNode.get(0).formFieldPath).toBe(expectedSubFormFieldPath);
      expect(subNode.get(0).control.concept.uuid).toBe(obsConcept.uuid);
      expect(subNode.get(0).value.value).toBe(1);
    });

    it('should update data from record when input obs is empty', () => {
      const emptyObservation = [];
      const expectedSubFormFieldPath = 'SingleGroup.3/3-0/4-0';
      const controlRecordTree = new ControlRecordTreeBuilder().build(
        metadata,
    ...
    it('should generate data from record when input obs is empty', () => {
      ...
      expect(obs).toHaveLength(1);
      expect(obs[0].groupMembers).toHaveLength(1);
      expect(obs[0].groupMembers[0].value).toBe(1);
    });
    ...
    it('should remove value of all child records when a parent record is removed', () => {
      ...
      const rootControl = new ControlRecord({
        children: List.of(parentControl),
      });
      const formFieldPath = 'ObsGroupTEst2.1/1-1';
      const updatedControlRecord = rootControl.remove(formFieldPath);
      expect(updatedControlRecord.getValue()).toBeUndefined();
      expect(updatedControlRecord.children.get(0).active).toBe(false);
      expect(updatedControlRecord.children.get(0).getValue()).toBeUndefined();
      expect(
        updatedControlRecord.children.get(0).children.get(0).getValue(),
      ).toBeUndefined();
    });
    ...
    it('should generate data from record when input obs is nested section', () => {
      ...
      expect(controlRecordTree.children.size).toBe(1);
      expect(
        controlRecordTree.children.get(0).children.get(0).value.value,
      ).toBe(23);

      const nested = controlRecordTree.children.get(0).children.get(1);
      expect(nested.children.size).toBe(1);
      expect(nested.children.get(0).value.value).toBe(1212);
    });
  });
  ...
  it('should generate record tree when section is add-more in given metadata with child obsControl', () => {
    ...
    expect(controlRecordTree.children.size).toBe(1);
    expect(controlRecordTree.children.get(0).children.get(0).value.value).toBe(
      120,
    );
  });

  it('should generate record tree when section is add-more in given metadata with child obsGroupControl', () => {
    const metadata = {
    ...
    expect(sectionRecord.formFieldPath).toBe(
      'section-add-more-with-obsGroup.1/1-0',
    );
    const obsGroupRecord = sectionRecord.children.get(0);
    expect(obsGroupRecord.formFieldPath).toBe(
      'section-add-more-with-obsGroup.1/1-0/2-0',
    );
    expect(obsGroupRecord.children.get(0).formFieldPath).toBe(
      'section-add-more-with-obsGroup.1/1-0/2-0/3-0',
    );
    expect(obsGroupRecord.children.get(0).value.value).toBe(120);
  });
});
